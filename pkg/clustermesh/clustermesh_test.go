...
func onIPIdentityChange(modType ipcache.CacheModification, oldIPIDPair *identity.IPIdentityPair, newIPIDPair identity.IPIdentityPair) {
}
...
func (s *ClusterMeshTestSuite) TestClusterMesh(c *C) {
	...
	cm, err := NewClusterMesh(Configuration{
		...
		IdentityChangeHandler: onIPIdentityChange,
	})
	c.Assert(err, IsNil)
